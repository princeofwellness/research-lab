...
const {data} = await useAsyncData(`post.${route.params.id}`, () => $fetch(`/api/blog/posts/${route.params.id}`));

const ogImage = data.value.image?.sizes?.large?.source_url ?? ogMetaImage;

useHead({
  title: `${data.value ? data.value.title : ''} | RESEARCH LAB`,
  meta: [
    {name: 'description', content: data.value?.excerpt ?? ogMetaDescription},
...
<template>
  <Header/>
  <div class="flex flex-col min-h-screen items-center  font-source-code-pro">
    <section class="grow flex flex-col text-techblue w-full px-[32px] max-w-screen-2xl py-[136px] ">

      <div class="border-2 border-techblue p-[68px]">
      <div class="flex w-full items-center">
        <NuxtLink to="/blog" class=" top-[21px] left-[21px] z-50">
          <button
              class="w-auto border-[1.75px] text-[32px] pb-1 font-medium text-[--bg-color] bg-techblue px-6 hover:bg-[--bg-color] hover:border-techblue hover:text-techblue transition-colors duration-300 focus:border-1">
            ←
          </button>
        </NuxtLink>
        <h2 class="pl-[32px] text-26 lg:text-68 font-semibold uppercase leading-tight">
          {{ data.title }}
        </h2>
      </div>

      <div id="content" class="pt-[32px] text-[16px] lg:text-[21px] editor-styles-wrapper" v-html="data.content"></div>
      </div>
    </section>
...
:root :where(.wp-block-pullquote p:last-of-type) {
...
:root :where(.wp-block-comment-edit-link a:where(:not(.wp-element-button))) {
	color: var(--wp--preset--color--contrast);
}

:root :where(.wp-block-comment-reply-link) {
	font-size: var(--wp--preset--font-size--small);
}

:root :where(.wp-block-comment-reply-link a:where(:not(.wp-element-button))) {
	color: var(--wp--preset--color--contrast);
}
...
:root :where(.wp-block-post-comments-form textarea, .wp-block-post-comments-form input:not([type=submit])) {
	border-radius: .25rem;
	border-color: var(--wp--preset--color--accent-6) !important;
}
...
:root :where(.wp-block-search .wp-element-button:hover, .wp-block-search .wp-block-button__link:hover) {
	border-color: transparent;
}
...
:root :where(.wp-block-term-description) {
	font-size: var(--wp--preset--font-size--medium);
}
...
:root{--wp--preset--duotone--dark-grayscale:url(#wp-duotone-dark-grayscale);--wp--preset--duotone--grayscale:url(#wp-duotone-grayscale);--wp--preset--duotone--purple-yellow:url(#wp-duotone-purple-yellow);--wp--preset--duotone--blue-red:url(#wp-duotone-blue-red);--wp--preset--duotone--midnight:url(#wp-duotone-midnight);--wp--preset--duotone--magenta-yellow:url(#wp-duotone-magenta-yellow);--wp--preset--duotone--purple-green:url(#wp-duotone-purple-green);--wp--preset--duotone--blue-orange:url(#wp-duotone-blue-orange);}
